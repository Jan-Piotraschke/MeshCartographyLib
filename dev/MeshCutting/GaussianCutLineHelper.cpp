...
GaussianCutLineHelper::GaussianCutLineHelper(pmp::SurfaceMesh& mesh, pmp::Vertex& start_vertex)
...
/**
 * @brief Based on the Gaussian curvature K = k1*k2 (k1 and k2 are the principal curvatures (minimum and maximum normal
 * curvature))
 */
pmp::Vertex GaussianCutLineHelper::get_gaussian_vertex()
{
    ...
    std::sort(
        ...
        sorted_vertices.end(),
        [&](const pmp::Vertex& a, const pmp::Vertex& b) { return curvatures[a] > curvatures[b]; });
