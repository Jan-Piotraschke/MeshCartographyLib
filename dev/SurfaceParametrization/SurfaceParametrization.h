...
class SurfaceParametrization
{
  public:
    ...
    std::tuple<std::vector<int64_t>, Eigen::MatrixXd, Eigen::MatrixXd, std::string> create_uv_surface(
        std::string mesh_file_path, int32_t start_node_int);

    std::string get_mesh_name(const std::string mesh_file_path);

    bool check_point_in_polygon(const Point_2_eigen& point, bool is_original_mesh);
  ...
  private:
    ...
    std::vector<int64_t> calculate_uv_surface(_3D::vertex_descriptor start_node);

    void save_UV_mesh(UV::Mesh _mesh, UV::halfedge_descriptor _bhd, _3D::UV_pmap _uvmap, const std::string mesh_path);
