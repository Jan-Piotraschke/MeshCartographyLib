...
TessellationDistanceHelper::TessellationDistanceHelper(
    fs::path mesh_path, std::vector<std::vector<int64_t>> equivalent_vertices)
    : mesh_path(mesh_path), equivalent_vertices(equivalent_vertices)
{
}
...
Eigen::MatrixXd TessellationDistanceHelper::get_mesh_distance_matrix()
{
    ...
    for (int i = 0; i < original_vertices_num; ++i)
    {
        ...
        dijkstra_distance_helper.fill_distance_matrix(mesh, distance_matrix_v, pmp::Vertex(i));
    }
