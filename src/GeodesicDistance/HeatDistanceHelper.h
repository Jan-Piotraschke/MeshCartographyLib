...
class HeatDistanceHelper : public GeodesicDistanceHelperInterface
{
  ...
  private:
    ...
    void fill_distance_matrix(Triangle_mesh mesh, Eigen::MatrixXd& distance_matrix, int closest_vertice);

    std::vector<double> geo_distance(Triangle_mesh mesh, int32_t start_node);
};
